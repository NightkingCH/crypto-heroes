...
if [[ $# -eq 0 ]] ; then
  exec geth --config .config.toml --allow-insecure-unlock --nousb --verbosity $ETH_VERBOSITY --identity "$ETH_IDENTITY" --rpc --rpcaddr "0.0.0.0" --rpcapi "admin,debug,web3,eth,txpool,personal,clique,miner,net" --rpccorsdomain="*" --gcmode=archive --mine --miner.threads 1 --unlock $ETH_ADDRESS --password /tmp/eth_pass
else
  exec "$@"
fi
